...
void ngenic::ngenic_displace_particles(void)
{
  ...
  for(Type = MinType; Type <= MaxType; Type++)
    ...
    {
#ifdef NGENIC_2LPT
    if(All.NLR == 0) { // do not let 2LPT be triggered if massive neutrinos are included
      ...
      for(int axes = 0; axes < 3; axes++)
        {
          ...
          ngenic_setup_modes_in_kspace((fft_complex *)disp, NULL);
      ...
      for(int axes = 0; axes < 3; axes++)
        {
          ...
          fft_real *vel;
          vel = (fft_real *)Mem.mymalloc_movable(&vel, "vel", maxfftsize * sizeof(fft_real));

          ngenic_setup_modes_in_kspace((fft_complex *)disp, (fft_complex *)vel);

          ngenic_get_derivate_from_fourier_field(axes, -1, (fft_complex *)disp);

          ngenic_get_derivate_from_fourier_field(axes, -1, (fft_complex *)vel);

/*for (int a=0; a<maxfftsize; a++) {
        printf("vel[%d] = %.12f, disp[%d] = %.12f\n", a, vel[a], a, disp[a]);
}*/
...
void ngenic::ngenic_setup_modes_in_kspace(fft_complex *fft_of_grid, fft_complex *fft_of_vel)
{
