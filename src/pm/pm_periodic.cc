  ...
  if(Nulinear.initialisation_switch != 1) {
    ...
    for(int bin_index=0; bin_index<PMGRID; bin_index++) {
      if(power_nuGadget[bin_index] != 0) {
        ...
        double fnu_pm = All.OmegaNu/(All.OmegaNu + All.Omega0);
...
*/
    // write output file for neutrino delta, when power spectrum output is requested.
    if(mode != 0) {
      if(ThisTask == 0){
...
void pm_periodic::pmforce_measure_powerspec_inline(int flag, int *typeflag, double *k_out, double *power_out)
{
  particle_data *P = Sp->P;

  long long CountModes[PMGRID];
  double SumPowerUncorrected[PMGRID]; /* without binning correction (as for shot noise) */
  double PowerUncorrected[PMGRID];    /* without binning correction */
  double DeltaUncorrected[PMGRID];    /* without binning correction */
  double ShotLimit[PMGRID];
  double KWeightSum[PMGRID];
  double Kbin[PMGRID];
 ...
 *                                  */
  ...
  MPI_Allreduce(MPI_IN_PLACE, SumPowerUncorrected, PMGRID, MPI_DOUBLE, MPI_SUM, Communicator);
  MPI_Allreduce(MPI_IN_PLACE, CountModes, PMGRID, MPI_LONG_LONG, MPI_SUM, Communicator);
  MPI_Allreduce(MPI_IN_PLACE, KWeightSum, PMGRID, MPI_DOUBLE, MPI_SUM, Communicator);
  ...
  for(int i=0; i<PMGRID; i++) {
    k_out[i] = Kbin[i];
    power_out[i] = PowerUncorrected[i]*All.BoxSize*All.BoxSize*All.BoxSize;
