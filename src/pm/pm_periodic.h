...
#define POWERSPEC_FOLDFAC 16 /* folding factor to obtain an estimate of the power spectrum on very small scales */
  ...
  char neutrino_delta_fname[MAXLEN_PATH_EXTRA];

  int NSource;

  void pmforce_measure_powerspec_inline(int flag, int *typeflag, double *k_out, double *power_out);
 ...
 public:
  ...
  {
    return pow(a / ((All.Omega0 + All.OmegaNu) + (1 - All.Omega0 - All.OmegaNu - All.OmegaLambda) * a + All.OmegaLambda * a * a * a), 1.5);
  }
  ...
  double linear_growth(double a)
  {
    ...
    double hubble_a = Driftfac.hubble_function(a);
